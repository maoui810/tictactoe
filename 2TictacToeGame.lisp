...
(defun connect-3 (&key (gameBoard (make-array '(16) :initial-contents '(_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _))))
	...
	(setf (aref gameBoard 0) *AISymbol*)
	(setf (aref nums 0) *AISymbol*)
	(setf (aref gameBoard 2) *AISymbol*)
	(setf (aref nums 2) *AISymbol*)
	...
	(setq temp-board (copy-seq gameBoard))
	 ...
	 (setq a (MIN-VALUE temp-board))
	(print "best value")
	(print a)
	
	
	(print  "Min BEST ACTION TO TAKE" )
	(print minTemList)
	
	(setf temp-board2(make-array (length gameBoard)))
	(setq temp-board2 (copy-seq gameBoard))
	
	(setq b (MAX-VALUE temp-board2))
	(print  "MAX BEST ACTION TO TAKE" )
	(print maxTemList)
	(print "best value for MAX")
	(print b)
	
	
	;(setq bestmove (car minTemList))
	;(setf (aref gameBoard bestmove) *AISymbol*)
	;(print "TEMP LIST")
	;(print maxTemList)
