...
(defun MINIMAX-DECISION (gameBoard)
	...
	(setq action_list (ACTION gameBoard))
	
	(setq best_action (car action_list))
	
	(setq best_score (MIN-VALUE(gameBoard)))
...
(defun MAX-VALUE(gameBoard)
	...
	(loop for x in (ACTION gameBoard)
		do(if (eq check 1)
			...
			(setq bb (list x val))
			(push x TEMPLIST))
			(setq val (min val (MIN-VALUE(RESULT gameBoard x *AISymbol*)))))
			
	)
	(setq maxTemList TEMPLIST)
	(print "makxxxx  value ")
	(print val)
...
(defun connect-3 (&key (gameBoard (make-array '(16) :initial-contents '(_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _))))
	...
	(setq m (car mov))
	(print m)
	(setf (aref gameBoard m) *AISymbol*)
